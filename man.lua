...
function M.show_toc()
  ...
  while lnum and lnum < last_line do
    ...
    elseif flag_title_re:match_str(text) then
      -- if text is a flag title
      local is_same_indent = (fn.indent(lnum) == fn.indent(lnum - 1))
      local is_prev_line_flag_title = (prev_flag_title_lnum == lnum - 1)
      -- avoid some false positive: --flag appears within a wrapped paragraph
      if is_same_indent == is_prev_line_flag_title then
	-- we strip whitespaces and prepend two
